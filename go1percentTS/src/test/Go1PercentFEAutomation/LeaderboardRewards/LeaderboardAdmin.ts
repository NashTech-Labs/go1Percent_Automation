...
const RewardsPageUIAutomationAdmin: NightwatchTests = {
    ...
    "Should display rewards tab on expanding leaderboard": () => {
        sessionsPage
    ...
    "Should display rewards page on clicking rewards button": () => {
        sessionsPage
        .waitForElementPresent('@rewardsButton')
    ...
    "Should be able to see all rewards for admin on scrolling": (browser: NightwatchBrowser) => {
        browser
        .execute(function() {
    ...
    "Should be able to switch between forCompetency from Individual Buttons": () => {
        sessionsPage
    ...
    "Should be able to open update rewards page on clicking edit button": () => {
    ...
    "Should be able to see delete popup for cross button": () => {
        const title = "Delete"
        sessionsPage
   ...
   // Verify that admin should be able to edit Reward description (TC-289) 
    "Should be able to see message on updating reward description": () => {
        sessionsPage
    ...
    "Should be able to see message on updating reward name": () => {
        sessionsPage
    ...
    "Should be able to increase or decrease required points": () => {
        ...
        sessionsPage
           ...
           .getValue("@requiredPoints",function (result) {
            const currentValue = typeof result.value === 'string' ? parseInt(result.value, 10) : 0;
            updatedValue = currentValue + 1;
            this.setValue("input[type='number']",updatedValue.toString())
           })
            .clickOnUpdateButton()
            .assert.textContains('@updateMessage', globalsData.rewardSectionMessages.successMessage)
        },
    ...
    "Should be able to increase or decrease stock quantity for admin": () => {
        let updatedValue: number;
        sessionsPage
       .waitForElementPresent('@editButton',8000)
       .clickOnEditButton()
       ...
       .getValue('@stockQuantity', function (result) {
        ...
        .perform((done: () => void) => {
            ...
            .setValue('@stockQuantity', updatedValue.toString())
    ...
    "Should be able to see message on clicking cross button on image": () => {
        sessionsPage
           .waitForElementPresent('@editButton',6000)
           .clickOnEditButton()
    ...
    "Should be able to upload new image to existing reward image": () => {
   ...
   //  Verify that admin should not be able to add any other extension files (TC-287)
    "Should be able to see popup message on uploading invalid format image": () => {
    ...
    //  Verify that admin should not be able to add any image files of invalid size            
     "Should be able to see popup message on uploading image of invalid size": () => {
...
export default RewardsPageUIAutomationAdmin;