 ...
 */
function assertResponseTime(startTime: number, maxTime: number): void {
...
describe('api testing ', () => {
  it('token generation', async function () {
    ...
    const response = await request(tokenUrl)
      ...
      .set(config.admin.tokenHeaders)
      .send(config.admin.requestData)
      .expect(200)
      .then((response) => {
        const token = response.body.access_token;
        config.employee.headers['Authorization'] = 'Bearer ' + token;
      });

  })

  it('should retrieve Leading Practices Summary', async () => {
    const startTime = performance.now();
    const response = await request(config.admin.base_url)
      .get('/studios')
      .set(config.employee.headers)
   ...
   */
  it('should retrieve Leading Nasher data for all times', async function () {
    const startTime = performance.now();
  ...
  * @param {Object} supertest - The supertest object for making HTTP requests.
  */
  it('should retrieve Leading Nasher data monthly', async function () {
    const startTime = performance.now();
...
* @param {Object} supertest - The supertest object for making HTTP requests.
*/
  it('should retrieve Leading Nasher data', async function () {
 ...
 * @param {Object} supertest - The supertest object for making HTTP requests.
 */
  it('should retrieve Leading Nasher of specific month', async function () {
    const startTime = performance.now();
   ...
   * @param {Object} supertest - The supertest object for making HTTP requests.
   */
  it('should retrieve user profile activities', async function () {

    const startTime = performance.now();
    
    const response = await request('https://feed-service.qa.go1percent.com/')
    ...
    for (let indexOfResponse = 0; indexOfResponse < responseBody.length; indexOfResponse++) {
      ...
      expect(object.feedcomment).to.satisfy((value: null) => typeof value === 'string' || value === null);
  ...
  it('should handle profile picture not found', async function () {
  ...
  it('should find all the rewords', async function () {

    const startTime = performance.now();

    request(config.admin.base_url)
      ...
      .then((response) => {
        const responseBody = response.body;
        assertResponseTime(startTime, 5000);
    ...
    // Assert that the execution time is less than or equal to 300 milliseconds.
  ...
  it('verify the points in points table', async function () {
    ...
    expect(responseBody.knolderName).to.be.a('string');
